...
def get_auto_trader_dashboard():
    ...
    # Active positions from bot instance
    active_list = list(auto_trader.active_positions.values())

    # Trade history from bot instance
    history_list = getattr(auto_trader, "trade_history", [])

    # Real logs from bot instance
    logs = getattr(auto_trader, "logs", [])
    if not logs:
        now_str = datetime.now().strftime("%H:%M:%S")
        logs = [
            f"[{now_str}] [System] KIS 자동 매매 봇 감시 대기 중 (모드: {'모의' if Config.KIS_IS_PAPER else '실전'})",
        ]
